...
3. Speedup comparison (slowest component vs reference) for each log
...
def plot_matching_and_failures(summary_data: dict, results_dir: Path = None, output_file: str = None):
    ...
    fully_matching = 0  # Combined components match reference
    not_matching = 0  # Combined components don't match reference
    has_failures = 0  # At least one component failed
    ...
    extra_event_types = {}  # event_type -> count (events only in component)
    missing_event_types = {}  # event_type -> count (events only in reference)
    differing_block_types = {}  # block_type -> count (for info only)
    
    # Track per-log event difference counts
    per_log_extra_counts = {}  # log_name -> count
    per_log_missing_counts = {}  # log_name -> count

    for log_result in log_results:
        log_name = log_result['log_file'].replace('.log', '').replace('minitwit_', '')
        log_names.append(log_name)
        per_log_extra_counts[log_name] = 0
        per_log_missing_counts[log_name] = 0
        ...
        if results_dir:
            ...
            # Load the combined components diff file for detailed comparison data
                ...
                try:
                    ...
                    for block in differing_blocks:
                        ...
                        for event_str in only_in_partition:
                            event_key = event_str.strip()
                            extra_event_types[event_key] = extra_event_types.get(event_key, 0) + 1
                            total_extra_events += 1
                        
                        for event_str in only_in_reference:
                            event_key = event_str.strip()
                            missing_event_types[event_key] = missing_event_types.get(event_key, 0) + 1
                    ...
                    per_log_extra_counts[log_name] = total_extra_events
                    per_log_missing_counts[log_name] = total_missing_events
    ...
    ax2.legend(handles=legend_elements, loc='upper center', bbox_to_anchor=(0.5, -0.13),
    ...
    # Print event summary with per-log statistics
    all_extra_counts = list(per_log_extra_counts.values())
    all_missing_counts = list(per_log_missing_counts.values())

    print(f"\nEvent difference summary (from differing blocks):")
    print(f"  Extra events (with args): {extra_event_types}")
    print(f"  Missing events (with args): {missing_event_types}")
    print(f"\nPer-log event difference counts:")
    for lname in log_names:
        extra = per_log_extra_counts.get(lname, 0)
        missing = per_log_missing_counts.get(lname, 0)
        if extra > 0 or missing > 0:
            print(f"  {lname}: {extra} extra, {missing} missing")
    if all_extra_counts:
        print(f"\nAverages across all {len(all_extra_counts)} logs:")
        print(f"  Avg extra events per log:   {np.mean(all_extra_counts):.1f} (±{np.std(all_extra_counts):.1f})")
        print(f"  Avg missing events per log: {np.mean(all_missing_counts):.1f} (±{np.std(all_missing_counts):.1f})")
        print(f"  Logs with extra events:   {sum(1 for v in all_extra_counts if v > 0)}/{len(all_extra_counts)}")
        print(f"  Logs with missing events: {sum(1 for v in all_missing_counts if v > 0)}/{len(all_missing_counts)}")

    # Return event data for separate plotting
...
def plot_event_type_differences(extra_event_types: dict, missing_event_types: dict, output_file: str = None):
    """
    Plot event differences (with full arguments) from differing blocks.

    Args:
        extra_event_types: Dictionary of full event strings and counts only in component
        missing_event_types: Dictionary of full event strings and counts only in reference
        output_file: Optional path to save the plot
    """
    has_event_data = bool(extra_event_types or missing_event_types)

    if not has_event_data:
        print("No event differences to plot")
        return

    def truncate_label(label, max_len=55):
        """Truncate a label to max_len characters for display."""
    ...
    def make_colors(n, _cmap_hint):
        # Use qualitative colormaps for distinct, saturated slice colors
        cmap = plt.colormaps.get_cmap('tab20' if n > 10 else 'tab10')
        return [cmap(i % cmap.N) for i in range(n)]

    # Create figure with two pie charts side by side
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(18, 8))

    # Plot 1: Extra events (only in component, not in reference)
    if extra_event_types:
        labels_extra_raw = list(extra_event_types.keys())
        sizes_extra = list(extra_event_types.values())
        labels_extra = [truncate_label(l) for l in labels_extra_raw]
        colors_extra = make_colors(len(labels_extra), 'Blues')  # hint ignored, uses tab colormap
        fsize = get_font_size(len(labels_extra))

        ax1.pie(sizes_extra, labels=labels_extra, colors=colors_extra,
                autopct=lambda pct: f'{pct:.1f}%\n({int(round(pct / 100 * sum(sizes_extra)))})',
                shadow=False, startangle=90,
                textprops={'fontsize': fsize})
        ax1.set_title(f'Extra Events in Component\n(Total: {sum(sizes_extra)} events not in reference)',
                      fontsize=13, fontweight='bold')
    else:
        ax1.text(0.5, 0.5, 'No Extra Events', ha='center', va='center',
                 fontsize=14, fontweight='bold', transform=ax1.transAxes)
        ax1.set_title('Extra Events in Component', fontsize=13, fontweight='bold')
        ax1.axis('off')

    # Plot 2: Missing events (only in reference, not in component)
    if missing_event_types:
        labels_missing_raw = list(missing_event_types.keys())
        sizes_missing = list(missing_event_types.values())
        labels_missing = [truncate_label(l) for l in labels_missing_raw]
        colors_missing = make_colors(len(labels_missing), 'Reds')  # hint ignored, uses tab colormap
        fsize = get_font_size(len(labels_missing))

        ax2.pie(sizes_missing, labels=labels_missing, colors=colors_missing,
                autopct=lambda pct: f'{pct:.1f}%\n({int(round(pct / 100 * sum(sizes_missing)))})',
                shadow=False, startangle=90,
                textprops={'fontsize': fsize})
        ax2.set_title(f'Missing Events in Component\n(Total: {sum(sizes_missing)} events not in component)',
                      fontsize=13, fontweight='bold')
    else:
        ax2.text(0.5, 0.5, 'No Missing Events', ha='center', va='center',
                 fontsize=14, fontweight='bold', transform=ax2.transAxes)
        ax2.set_title('Missing Events in Component', fontsize=13, fontweight='bold')
        ax2.axis('off')
...
def plot_speedup_comparison(summary_data: dict, results_dir: Path = None, output_file: str = None, component_names: dict = None):
    """
    Plot speedup between slowest component and reference for each log.
    
    Args:
        summary_data: Multi-log summary JSON data
        results_dir: Optional base directory for loading detailed enforcement results
        output_file: Optional path to save the plot
        component_names: Optional dict mapping component filenames to custom names
    """
    log_results = summary_data['log_results']
    ...
    accumulated_step_contributors = []  # Track which components contributed to each step
    num_components = 0  # Will be set from partition_details when available
    ...
    for log_result in log_results:
        ...
        if timing and 'reference_comparison' in timing:
            ...
            if speedup:
                ...
                contributors = None  # Track which components contributed to accumulated worst-case
                ...
                if results_dir:
                    ...
                    if enforcement_file.exists():
                        try:
                            ...
                            # Track component count from partition_details
                            successful_parts = [p for p in enforcement_data.get('partition_details', []) if p.get('status') == '✓ SUCCESS']
                            if len(successful_parts) > num_components:
                                num_components = len(successful_parts)
                            
                            # Collect step timings from partition_details in enforcement_results.json
                            step_max_times = {}  # timepoint -> (max_time, partition_file)
                            component_step_totals = []  # Total time from step-by-step for each component
                            ...
                            for partition_detail in enforcement_data.get('partition_details', []):
                                ...
                                if total_time_stats:
                                    component_step_totals.append(total_time_stats['mean'])
                                ...
                                for step in step_timing.get('steps', []):
                                    ...
                                    if timepoint not in step_max_times:
                                        ...
                                        # Update if this component has a slower time for this step
                            ...
                            if step_max_times:
                                ...
                                # Create a summary of which components contributed
                                contributors = {}
                                for timepoint, (step_time, part_file) in sorted(step_max_times.items()):
                                    # Use custom name if provided, otherwise simplify component name
                                    if component_names and part_file in component_names:
                                        part_name = component_names[part_file]
                                    else:
                                        part_name = part_file.replace('mfotl_', 'C').replace('.mfotl', '')
                                    if part_name not in contributors:
                                        contributors[part_name] = []
                                    contributors[part_name].append(timepoint)
                            
                            # Use slowest component's step-based total time if available
                            if component_step_totals:
                                slowest_step_based_time = max(component_step_totals)
                        except Exception as e:
    ...
    if has_accumulated_data:
        # Plot both slowest component and accumulated worst-case speedups
        width = 0.35
        ...
        for i, (bar, speedup) in enumerate(zip(bars1, speedups)):
            ...
            text_color = '#1a7a3c' if speedup > 1 else '#e74c3c'
        ...
        # Original behavior - just slowest component
    ...
    ax.set_xlabel('Speedup Factor (reference time / component time)', fontsize=11, fontweight='bold')
    title_str = f'Speedup ({num_components} components)' if num_components > 0 else 'Speedup'
    ...
    # Print statistics
    print("\n=== Speedup Statistics (Slowest Component) ===")
...
def plot_execution_time_comparison(summary_data: dict, results_dir: Path = None, output_file: str = None, component_names: dict = None):
    """
    Plot execution time comparison between slowest component and reference for each log.
    ...
    for log_result in log_results:
        ...
        if timing and 'reference_comparison' in timing:
            ...
            if speedup:
                ...
                if results_dir:
                    ...
                    if enforcement_file.exists():
                        try:
                            ...
                            # Collect step timings from partition_details in enforcement_results.json
                            step_max_times = {}  # timepoint -> (max_time, partition_file)
                            partition_step_totals = []  # Total time from step-by-step for each component
                ...
                # Use step-based time if available, otherwise use component-level time
                if slowest_step_based_time is not None:
    ...
    if has_accumulated_data:
        # Three bars: slowest component, accumulated worst-case, reference
        width = 0.25
        bars1 = ax.bar(x - width, slowest_times, width, label='Slowest Component', 
                        color='#3498db', edgecolor='black', linewidth=1.2)
        ...
        # Two bars: slowest component and reference (original behavior)
        width = 0.35
        bars1 = ax.bar(x - width/2, slowest_times, width, label='Slowest Component', 
                        color='#3498db', edgecolor='black', linewidth=1.2)
    ...
    ax.legend(fontsize=10, loc='upper center', bbox_to_anchor=(0.5, -0.16), ncol=3)
    ...
    if has_accumulated_data:
        ...
        for i, log_name in enumerate(log_names):
            if accumulated_worst_case_times[i] is not None:
                print(f"{log_name}:")
                print(f"  Slowest component: {slowest_times[i]:.6f}s")
                ...
                # Show which components contributed to accumulated worst-case
                if accumulated_step_contributors[i] is not None:
                    contrib = accumulated_step_contributors[i]
                    # Format as: component_name (steps X, Y, Z)
...
def plot_complexity_vs_time_all_logs(summary_data: dict, results_dir: Path, 
    ...
    """
    Plot 3: Complexity vs time across all logs and all components.
    
    Args:
        summary_data: Multi-log summary JSON data
        results_dir: Base directory containing all log subdirectories
        mfotl_dir: Directory containing component MFOTL files
        output_file: Optional path to save the plot
    """
    log_results = summary_data['log_results']
    
    # Collect data from all components across all logs
    partition_data = []
    ...
    if not partition_data:
        print("No component data to plot")
        return
    
    print(f"\nAnalyzing {len(partition_data)} components across {len(log_results)} logs")
    ...
    # Check if any component has variance (std > 0)
    ...
    if len(num_lets_list) > 1 and len(set(num_lets_list)) > 1:
        ...
        ax2.plot(x_trend, p(x_trend), "r--", alpha=0.7, linewidth=2.5, 
            label=f'Trend: y={z[0]:.3f}x+{z[1]:.3f}')
        ax2.legend(fontsize=10, loc='upper center', bbox_to_anchor=(0.5, -0.16), frameon=True)
...
def main():
    ...
    parser.add_argument('mfotl_dir',
                       help='Directory containing component MFOTL files')
    ...
    if args.component_names:
        component_names_path = Path(args.component_names)
        if not component_names_path.exists():
            print(f"Error: Component names file not found: {component_names_path}")
            return 1
        try:
            with open(component_names_path, 'r') as f:
                component_names = json.load(f)
            print(f"Loaded {len(component_names)} component name mappings")
        except Exception as e:
    ...
    plot_speedup_comparison(summary_data, results_dir, output_2a, component_names)
    
    print("\n=== Generating Plot 3b: Execution Time Comparison ===")
    plot_execution_time_comparison(summary_data, results_dir, output_2b, component_names)
