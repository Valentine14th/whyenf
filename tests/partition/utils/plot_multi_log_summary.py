...
def plot_speedup_comparison(summary_data: dict, output_file: str = None):
    """
    ...
    # Plot 2a: Speedup bar chart
    y_pos = np.arange(len(log_names))
    colors = ['#2ecc71' if s > 1 else '#e74c3c' for s in speedups]
    
    bars = ax1.barh(y_pos, speedups, color=colors, edgecolor='black', linewidth=1.2)
    ax1.axvline(x=1, color='black', linestyle='--', linewidth=2, label='No speedup (1x)')
    ax1.set_yticks(y_pos)
    ax1.set_yticklabels(log_names, fontsize=9)
    ax1.set_xlabel('Speedup Factor (reference time / slowest partition time)', fontsize=11, fontweight='bold')
    ax1.set_title('Speedup: Slowest Partition vs Reference', fontsize=13, fontweight='bold')
    ax1.invert_yaxis()
    ...
    # Plot 2b: Time comparison (slowest partition vs reference)
    x = np.arange(len(log_names))
    width = 0.35
    
    bars1 = ax2.bar(x - width/2, slowest_times, width, label='Slowest Partition', 
                    color='#3498db', edgecolor='black', linewidth=1.2)
    bars2 = ax2.bar(x + width/2, reference_times, width, label='Reference', 
    ...
    ax2.set_title('Execution Time: Slowest Partition vs Reference', fontsize=13, fontweight='bold')
    ax2.set_xticks(x)
...
def main():
    ...
    plot_speedup_comparison(summary_data, output_2)
